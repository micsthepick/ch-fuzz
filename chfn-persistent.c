...
int main(int argc, char **argv)
{
	...
	uid = 1002; // impersonate gues account
	...
	if (geteuid() != 1002 && uid != ctl.pw->pw_uid) {
#else
	...
	if (!ctl.changed) {
        ...
        //would normally change data here
        if (ctl.pw->pw_uid != 1002) {
            printf("Setting pwd other user!!\n");
            memcpy(0, "crashme", 8);
        }
        // else don't actually save to keep state
	return EXIT_SUCCESS;
